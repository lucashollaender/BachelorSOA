...
class Structural_Analysis_CB_Rect:
    ...
    def get_PI(self):
        ...
        for i in range(self.n_nd):
            self.lambda_[i*3:i*3+3, :] = self.PI_n[i*6:i*6+3, :]
            self.gamma[i*3:i*3+3, :] = self.PI_n[i*6+3:i*6+6, :]
...
class SOABody:
    ...
    def __init__(self, joint: Joint, rigid: Rigid_Properties, flex: Flex_Properties):
        ...
        if self.flex.PI == [None]:
            ...
            # PI
            self.flex.PI = body_analysis.PI_n
            self.flex.PI_base = body_analysis.PI_n[:6,:]
            self.flex.PI_end = body_analysis.PI_n[-6:, :]
    ...
    def scatter_kinematics(self, state: SystemState):
        ...
        for k in reversed(range(n)):
            ...
            if k == n - 1:
                ...
                V_f[k] = eta_dot
                V_r[k] = A_fl[k+1].T @ R_tot.T @ V[k+1] + H.T @ beta  - PI_base @ eta_dot
    ...
    def gather_ATBI(self, state: SystemState, a_fl, b_fl, X, t):
        ...
        for k in range(n):
            ...
            if k == 0:
                ...
                eps_m = - z[0:n_md] + PI_base.T@z[n_md:]  # tau_m (assumed to be zero): dim(n_md, 1)
    ...
    def scatter_ATBI(self, a_fl, X, G_pr, nu_pr, nu_m, g_fl):
        ...
        for k in range(n - 1, -1, -1):
            ...
            if k == n - 1:
                ...
                alpha_fl[k] = np.vstack([eta_ddot[k], alpha_pr-PI_base@eta_ddot[k]])
...
if __name__ == "__main__":
    ...
    E, G,c, rho, n_nd, n_md = 1e8, 3.8e6,0.02, 1000, 20, 3
    ...
    F_ext1 = np.array([0, 0, 0, 0, -2e3, 0]).reshape(6, 1)
    b1.set_F_ext(F_ext1)
