...
def build_system_data(
    ...
    J_diag=np.array([3.542, 3.542, 0.4167]),
    mass: float = 10.0,
):
    """
...
def make_initial_state(n):
    ...
    # identity quaternion for the rest
...
def animate_chain(S, t=None, stride=1, n=None, link_vecs=None):
    """
    S shape: (T, 7*n) expected for this n-link spherical-joint model (n quaternions + 3n omegas)

    t: array of time points (optional). If provided, animation runs at approximate real-time speed.
    ...
    all_pts = np.array(all_pts)  # shape (F, n+1, 3)
    ...
    # Simplified timing: use median dt (in ms) times stride
    if t is not None and len(all_times) > 1:
    ...
    ani = FuncAnimation(fig, update, frames=len(all_pts), init_func=init,
                        blit=False, interval=dt_ms, repeat=True)
